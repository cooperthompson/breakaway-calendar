...
class Command(BaseCommand):
    ...
    def ConvertPDFToText(self, pdf_file, layout):
        ...
        # fake the pdftotext conversion for windows
        # this assumes that you've manually downloaded files from linux
        # and stuck them in the /import/ directory
        if os.name == "nt":
    ...
    def get_games_non_layout(self, text_file):
        self.stdout.write("Loading games from %s" % text_file.name)
        mode = "start"
    ...
    def get_teams(self, text_file):
        self.stdout.write("Loading teams from %s" % text_file.name)
        mode = "start"
    ...
    def save_team(self, team_id, team_name):
        ...
        if match:
            ...
            self.stderr.write("No name/color found for %s." % team_name)
